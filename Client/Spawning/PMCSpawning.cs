...
{
    private static BotBossSpawn _bossSpawner;
    ...
    public static async Task InitializePmcSpawner(BotBossSpawn bossSpawner, BotSpawner botSpawner, IBotCreator botCreator)
    {
        AllPmcGroups.Clear();
    ...
    {
        BotBossSpawn.CG_Class332 @class = new BotBossSpawn.CG_Class332();
        @class.BotBossSpawn = _bossSpawner;
        ...
        GetProfileDataParams botProfileDataClass = new GetProfileDataParams(EPlayerSide.Savage, @class.wave.BossType, @class.wave.BossDif, time, @class.spawnParams);
        ...
        @class.spawnProcessData = new BotBossSpawn.BossSpawnProcess(@class.wave, @class.botZone, spawnPoint);
        _bossSpawner._spawnProcess.Add(@class.spawnProcessData);
        ...
        if (flag)
        {
            var canSpawn = _bossSpawner._spawner.CanSpawnRole(botProfileDataClass);
            if (Plugin.DebugLogging)
    ...
    private static async Task SpawnFollowers(BotCreationData bossCreationData, BotZone zone, int followersCount, BotSpawnParams spawnParams, BossLocationSpawn wave, EPlayerSide side, List<ISpawnPoint> pointsToSpawn, bool forceSpawn, string leaderProfileId)
    {
        ...
        {
            BotCreationData botCreationDataClass = await BotCreationData.Create(new GetProfileDataParams(EPlayerSide.Savage, wave.EscortType, wave.EscortDif, wave.Time, spawnParams, false), _iBotCreator, followersCount, _botSpawner);
    ...
    private static BotsGroup GetGroupAndSetEnemies(BotOwner bot, BotZone zone)
    {
        ...
        {
            foreach (BotOwner botOwner in _botSpawner.GetBotEnemiesList(bot))
            {
