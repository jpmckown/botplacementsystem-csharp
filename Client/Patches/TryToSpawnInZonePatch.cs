    ...
    private static void PatchPrefix(BotSpawner __instance, BotZone botZone, BotCreationData data, bool withCheckMinMax, bool newWave, ref List<ISpawnPoint> pointsToSpawn, bool forcedSpawn = false)
    {
        try
        {
            ...
            if (!isSmallMap)
            {
                ...
                Plugin.LogSource.LogInfo($"{data.Profiles[0].Nickname} - {botZone.NameZone} - Returning null points, no valid points in distance");
                pointsToSpawn = null;
            }
        }
        catch (Exception ex)
    ...
    private static float GetDistanceForMap(string mapName)
    {
        ...
        if (distance < 0f)
        {
            if (Plugin.DebugLogging)
                Plugin.LogSource.LogInfo($"TryToSpawnInZonePatch | GetDistanceForMap | Unknown map '{mapName}' - falling back to 10f");
            return 10f;
        }
