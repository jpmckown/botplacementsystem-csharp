...
internal class NonWavesSpawnSystemPatch : ModulePatch
{
        ...
        ref float ____lastCheckTime)
    {
        ...
        ref WDictionary<BotDifficulty> difficultyWeights = ref ____botDifficultiesWights;
        ...
        if (nextWindowToggleTime == null || nextWindowToggleTime <= ____game.PastTime)
        {
        ...
        ____botsController.ActivateBotsByWave(new SpawnWave
        {
            BotsCount = 1,
            Time = Time.time,
            Difficulty = ____botDifficultiesWights.Random(),
